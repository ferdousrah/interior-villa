...
const HospitalClinicInterior = (): JSX.Element => {
    ...
    const fetchService = async () => {
      try {
        const res = await fetch(
          "https://interiorvillabd.com/api/services/13?depth=1&draft=false&locale=undefined"
        );
  ...
  const metaTitle = seoDetails?.metaTitle || "Hospital & Clinic Interior Design in Bangladesh | Modern Workspaces";
  ...
  return (
    <main className="flex flex-col w-full items-start relative bg-white overflow-x-hidden min-h-screen">
      ...
      <SEO
        ...
        image={heroImage}
        url="https://interiorvillabd.com/services/commercial-interior/hospital-and-clinic-interior-design"
        type="service"
      ...
      <PageHero
        title={hero?.title || "Hospital & Clinic Interior Design"}        
        bgImage={heroImage}
        breadcrumbs={[
          { label: "Home", href: "/" },
          { label: "Services" },
          { label: "Commercial", href: "/services/commercial-interior" },
          { label: hero?.title || "Hospital & Clinic Interior Design", isActive: true },
        ]}
      />
...
export default HospitalClinicInterior;

...
const SpaAndBeautyParlorInterior = (): JSX.Element => {
    ...
    const fetchService = async () => {
      try {
        const res = await fetch(
          "https://interiorvillabd.com/api/services/16?depth=1&draft=false&locale=undefined"
        );
  ...
  const metaTitle = seoDetails?.metaTitle || "Spa & Beauty Parlor Interior Design in Bangladesh | Modern Workspaces";
  ...
  return (
    <main className="flex flex-col w-full items-start relative bg-white overflow-x-hidden min-h-screen">
      ...
      <SEO
        ...
        image={heroImage}
        url="https://interiorvillabd.com/services/commercial-interior/spa-and-beauty-parlor-interior-design"
        type="service"
      ...
      <PageHero
        title={hero?.title || "Spa & Beauty Parlor Interior Design"}        
        bgImage={heroImage}
        breadcrumbs={[
          { label: "Home", href: "/" },
          { label: "Services" },
          { label: "Commercial", href: "/services/commercial-interior" },
          { label: hero?.title || "Spa & Beauty Parlor Interior Design", isActive: true },
        ]}
      />
...
export default SpaAndBeautyParlorInterior;

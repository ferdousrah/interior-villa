...
const ResortInterior = (): JSX.Element => {
    ...
    const fetchService = async () => {
      try {
        const res = await fetch(
          "https://interiorvillabd.com/api/services/17?depth=1&draft=false&locale=undefined"
        );
  ...
  const metaTitle = seoDetails?.metaTitle || "Resort Interior Design in Bangladesh | Modern Workspaces";
  ...
  return (
    <main className="flex flex-col w-full items-start relative bg-white overflow-x-hidden min-h-screen">
      ...
      <SEO
        ...
        image={heroImage}
        url="https://interiorvillabd.com/services/commercial-interior/resort-interior-design"
        type="service"
      ...
      <PageHero
        title={hero?.title || "Resort Interior Design"}        
        bgImage={heroImage}
        breadcrumbs={[
          { label: "Home", href: "/" },
          { label: "Services" },
          { label: "Commercial", href: "/services/commercial-interior" },
          { label: hero?.title || "Resort Interior Design", isActive: true },
        ]}
      />
